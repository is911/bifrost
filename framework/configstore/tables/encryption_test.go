...
func TestTableKey_VertexUnresolvedSecretVar_RoundTrip(t *testing.T) {
	...
	key := &TableKey{
		...
		VertexKeyConfig: &schemas.VertexKeyConfig{
			ProjectID: schemas.SecretVar{
				Val:     "",
				SecretVar:  "env.FAKE_VERTEX_PROJECT_ID_FOR_TEST",
				FromEnv: true,
			},
			Region: *schemas.NewSecretVar("us-central1"),
		},
	...
	assert.Equal(t, "env.FAKE_VERTEX_PROJECT_ID_FOR_TEST", found.VertexKeyConfig.ProjectID.SecretVar,
...
func TestTableKey_AzureUnresolvedSecretVar_RoundTrip(t *testing.T) {
	...
	key := &TableKey{
		...
		AzureKeyConfig: &schemas.AzureKeyConfig{
			Endpoint: schemas.SecretVar{
				Val:     "",
				SecretVar:  "env.FAKE_AZURE_ENDPOINT_FOR_TEST",
				FromEnv: true,
			},
		},
	}
	...
	assert.Equal(t, "env.FAKE_AZURE_ENDPOINT_FOR_TEST", found.AzureKeyConfig.Endpoint.SecretVar,
...
func TestTableKey_BedrockUnresolvedSecretVar_RoundTrip(t *testing.T) {
	...
	key := &TableKey{
		...
		BedrockKeyConfig: &schemas.BedrockKeyConfig{
			AccessKey: schemas.SecretVar{
				Val:     "",
				SecretVar:  "env.FAKE_AWS_ACCESS_KEY_FOR_TEST",
				FromEnv: true,
			},
			SecretKey: schemas.SecretVar{
				Val:     "",
				SecretVar:  "env.FAKE_AWS_SECRET_KEY_FOR_TEST",
				FromEnv: true,
			},
			Region: schemas.NewSecretVar("us-west-2"),
		},
	...
	assert.Equal(t, "env.FAKE_AWS_ACCESS_KEY_FOR_TEST", found.BedrockKeyConfig.AccessKey.SecretVar,
		"env var reference for AccessKey lost on round-trip")
	assert.Equal(t, "env.FAKE_AWS_SECRET_KEY_FOR_TEST", found.BedrockKeyConfig.SecretKey.SecretVar,
...
func TestTableKey_OllamaUnresolvedSecretVar_RoundTrip(t *testing.T) {
	...
	key := &TableKey{
		...
		OllamaKeyConfig: &schemas.OllamaKeyConfig{
			URL: schemas.SecretVar{
				Val:     "",
				SecretVar:  "env.FAKE_OLLAMA_URL_FOR_TEST",
				FromEnv: true,
			},
		},
	}
	...
	assert.Equal(t, "env.FAKE_OLLAMA_URL_FOR_TEST", found.OllamaKeyConfig.URL.SecretVar)
...
func TestTableKey_SGLUnresolvedSecretVar_RoundTrip(t *testing.T) {
	...
	key := &TableKey{
		...
		SGLKeyConfig: &schemas.SGLKeyConfig{
			URL: schemas.SecretVar{
				Val:     "",
				SecretVar:  "env.FAKE_SGL_URL_FOR_TEST",
				FromEnv: true,
			},
		},
	}
	...
	assert.Equal(t, "env.FAKE_SGL_URL_FOR_TEST", found.SGLKeyConfig.URL.SecretVar)
