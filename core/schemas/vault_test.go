...
func TestStoreVaultSecretVar_NoOps(t *testing.T) {
	cases := []struct {
		...
		{"nil", nil},
		{"env-sourced", &SecretVar{FromEnv: true, SecretVar: "MY_VAR"}},
...
func TestStoreOwnedVaultSecretVars_WalksFields(t *testing.T) {
	...
	m := &model{
		...
		EnvBased: SecretVar{FromEnv: true, SecretVar: "X"},
	}
...
func TestStoreOwnedVaultEnvVars_WalksMap(t *testing.T) {
	stored := withStubVaultStore(t)

	type model struct {
		Headers map[string]EnvVar `gorm:"column:headers"`
	}
	m := &model{
		Headers: map[string]EnvVar{
...
func TestRemoveOwnedVaultEnvVars_WalksMap(t *testing.T) {
	var removed []string
	prev := VaultRemoveHook
	VaultRemoveHook = func(_ context.Context, path string) error {
		removed = append(removed, path)
