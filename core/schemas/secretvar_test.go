...
func TestSecretVar_UnmarshalJSON_SecretVarReference(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedSecretVar:  "env.TEST_API_KEY",
		...
		},
		{
			...
			expectedVal:     "",
			expectedSecretVar:  "env.NONEXISTENT_VAR",
			expectedFromEnv: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if secretVar.SecretVar != tt.expectedSecretVar {
				t.Errorf("Expected SecretVar=%q, got SecretVar=%q", tt.expectedSecretVar, secretVar.SecretVar)
			}
...
func TestSecretVar_UnmarshalJSON_FullStructure(t *testing.T) {
	...
	if secretVar.SecretVar != "env.MY_KEY" {
		t.Errorf("Expected SecretVar=%q, got SecretVar=%q", "env.MY_KEY", secretVar.SecretVar)
	}
	if !secretVar.FromEnv {
...
func TestNewSecretVar_SecretVarReference(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedSecretVar:  "env.TEST_NEW_ENVVAR_KEY",
			expectedFromEnv: true,
		},
		{
		...
		},
		{
			...
			expectedVal:     "",
			expectedSecretVar:  "env.MISSING_VAR",
			expectedFromEnv: true,
		},
	}
...
func TestSecretVar_Equals(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "equal values",
			a:        &SecretVar{Val: "test", SecretVar: "env.TEST", FromEnv: true},
			b:        &SecretVar{Val: "test", SecretVar: "env.TEST", FromEnv: true},
			expected: true,
		},
		{
...
func TestSecretVar_FullyRedacted(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:       SecretVar{Val: "resolved-secret", FromEnv: true, SecretVar: "env.PROXY_PASS"},
			wantVal:     "<REDACTED>",
			wantFromEnv: true,
			wantSecretVar:  "env.PROXY_PASS",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.SecretVar != tt.wantSecretVar {
				t.Errorf("SecretVar: want %q, got %q", tt.wantSecretVar, result.SecretVar)
			}
		})
	}
}
...
func TestSecretVar_IsSet(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    &SecretVar{SecretVar: "env.MISSING", FromEnv: true},
			expected: true,
		},
		{
			name:     "Val and SecretVar both set (env was resolved)",
			input:    &SecretVar{Val: "resolved-secret", SecretVar: "env.X", FromEnv: true},
			expected: true,
		},
		{
